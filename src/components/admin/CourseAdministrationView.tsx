...
export const CourseAdministrationView: React.FC = () => {
  ...
  const [adminCourseViewMode, setAdminCourseViewMode] = useState<'catalog' | 'hierarchy'>('catalog');
  ...
  return (
    <div className="space-y-6">
      ...
      {/* View Mode Toggle: Catalog vs 6-Level Hierarchy */}
      <div className="flex items-center justify-between gap-3 bg-white p-2 rounded-2xl border border-slate-200 shadow-xs flex-wrap">
        <div className="flex items-center gap-2">
          <button
            type="button"
            onClick={() => setAdminCourseViewMode('catalog')}
            className={`px-4 py-2 rounded-xl text-xs font-bold transition flex items-center gap-2 cursor-pointer ${
              adminCourseViewMode === 'catalog'
                ? 'bg-indigo-600 text-white shadow-xs'
                : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900'
            }`}
          >
            <BookOpen className="w-3.5 h-3.5" /> Course Catalog & Direct Cards
          </button>
          <button
            type="button"
            onClick={() => setAdminCourseViewMode('hierarchy')}
            className={`px-4 py-2 rounded-xl text-xs font-bold transition flex items-center gap-2 cursor-pointer ${
              adminCourseViewMode === 'hierarchy'
                ? 'bg-indigo-600 text-white shadow-xs'
                : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900'
            }`}
          >
            <FolderKanban className="w-3.5 h-3.5 text-emerald-400" /> 6-Level Course Hierarchy (Category &rarr; Course &rarr; Batch &rarr; Subject &rarr; Chapter &rarr; Lecture)
          </button>
        </div>

        <button
          onClick={handleOpenCreateModal}
          className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl text-xs transition cursor-pointer flex items-center gap-1.5 shadow-xs"
        >
          <Plus className="w-4 h-4" /> 17-Field Course Creator
        </button>
      </div>
      ...
      {/* ========================================================================= */}
      {/* MODAL 1: 17-FIELD COURSE CREATION MODAL */}
      {/* ========================================================================= */}
      <CourseCreationModal
